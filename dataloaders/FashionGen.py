...
class DataLoaderFashionGen(Dataloader):
    """
    Load fashion gen outfits data.
    """
    def __init__(self):
        super(DataLoaderFashionGen, self).__init__(path='data/fashiongen/')
    ...
    def init_phase(self, phase):
        ...
        feats_file = path_dataset + 'features_{}.npy'.format(phase)
        attr_file = path_dataset + '{}_attributes.json'.format(phase)
        np.random.seed(1234)
        ...
        with open(attr_file) as f:
            attrs = json.load(f)
            attrs['gender'] = np.array(attrs['gender'])
            attrs['category'] = np.array(attrs['category'])
            setattr(self, '{}_attributes'.format(phase), attrs)
    ...
    def get_phase(self, phase):
        ...
        attributes = getattr(self, '{}_attributes'.format(phase))
        ...
        genders = np.unique(attributes['gender'])
        cats = np.unique(attributes['category'])

        combined_gen_cat = {}
        for g in genders:
            for c in cats:
                key = '{}_{}'.format(g, c)
                combined_gen_cat[key] = list(set(np.where(attributes['gender'] == g)[0].tolist()) & set(np.where(attributes['category'] != c)[0].tolist()))

        for i in range(n_train_neg):
            print(i, n_train_neg, 'time elapsed: {}'.format(time.time() - before), end="\r")
            r_idx, c_idx = self.get_hard_negative_training_edge(lower_adj.shape[0], lower_adj, attributes, combined_gen_cat)
    ...
    def get_hard_negative_training_edge(self, num_nodes, lower_adj, attributes, combined_gen_cat):
        """
        Take into account gender and category information to sample the negatives:
        - no negatives from different gender (because they are obvious)
        - no negatives between same category
        """
        v = np.random.randint(num_nodes) # pick a random node
        v_gender = attributes['gender'][v]
        v_cat = attributes['category'][v]

        key = '{}_{}'.format(v_gender, v_cat)
        poss_nodes = combined_gen_cat[key]

        u = random.choice(poss_nodes)
